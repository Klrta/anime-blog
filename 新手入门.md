...
## 第四部分：最近的更新与问题修复记录 (2025年12月11日)

在开发过程中，我们遇到了一些常见的问题并进行了修复。这些经验对你理解 Next.js 项目非常有帮助。

### 1. 图片显示问题 (404 Not Found)
*   **现象**：文章里的图片加载不出来，控制台显示 404 错误。
*   **原因**：原本图片放在 `posts/images` 文件夹里，Markdown 里写的是 `./images/xxx.jpg`。但在 Next.js 中，只有 `public/` 文件夹里的资源才能被浏览器直接访问。
*   **修复**：
    1.  将图片全部移动到了 `public/life-log-images/` 文件夹。
    2.  使用正则批量替换，将 Markdown 里的 `./images/` 改为了 `/life-log-images/`。
*   **知识点**：**静态资源放 Public**。在 Next.js 里，如果你想通过 URL (比如 `http://localhost:3000/my-image.jpg`) 访问一张图，它必须在 `public` 文件夹里。

### 2. 文件读取报错 (EISDIR: illegal operation on a directory)
*   **现象**：运行项目时报错，提示无法读取目录。
*   **原因**：`src/lib/posts.ts` 里的代码尝试读取 `posts/` 文件夹下的所有东西。当我们在 `posts/` 里放了一个 `images` **文件夹**时，代码把它当成**文件**去读取，结果就报错了。
*   **修复**：修改了 `src/lib/posts.ts`，添加了过滤逻辑：
    ```typescript
    // 只处理以 .md 结尾的文件
    .filter(fileName => fileName.endsWith('.md'))
    ```

### 3. Markdown 格式清理 (Pandoc 遗留代码)
*   **现象**：从 Word/Mubu 转换来的 Markdown 里有很多奇怪的代码，比如 `{width="5.76in"}`，导致图片显示不正常。
*   **修复**：使用正则表达式清理了这些非标准的属性标记，让 Markdown 变回纯净的格式。

### 4. 界面统一与暗黑模式 (UI & Dark Mode)
*   **更新**：
    1.  引入了 `next-themes`，实现了**日夜间模式切换**。
    2.  **归档页面 (Archive)**：修复了背景不透明的问题。去掉了 `bg-anime-cream`，加上了 `relative z-10`，现在它和首页一样，能看到漂亮的滚动背景了。
    3.  **关于页面 (About)**：统一了导航栏样式，加上了毛玻璃效果和主题切换按钮。
*   **现状**：现在首页、归档页、关于页的视觉风格（导航栏、背景、卡片样式）已经完全统一。

### 5. 导航栏与布局统一 (UI Consistency)
*   **问题**：在不同页面（首页、归档、关于）之间切换时，导航栏的内容位置会发生微小的跳动。
*   **原因**：
    1.  **容器宽度不一致**：首页使用了 `max-w-6xl`，而归档页使用了 `max-w-5xl`。
    2.  **样式缺失**：首页的导航栏缺少暗黑模式的适配代码 (`dark:bg-black/30` 等)。
    3.  **间距不同**：不同页面的顶部内边距 (`padding-top`) 不一致，导致内容与导航栏的距离忽大忽小。
*   **修复**：
    1.  将所有页面的 Header 容器宽度统一设置为 `max-w-6xl`。
    2.  为首页 Header 补全了暗黑模式样式。
    3.  调整了归档页面的顶部间距 (`py-24`)，使其与关于页面保持一致。
*   **结果**：现在点击导航栏切换页面时，Logo 和菜单项的位置完全固定，不再跳动，体验更加丝滑。

### 6. 隐藏文章功能 (Hide Posts)
*   **需求**：有些文章（比如草稿或私人日志）不想在首页和归档页显示，但又不想删除文件。
*   **实现**：
    1.  修改了 `src/lib/posts.ts`，在获取文章列表时增加了过滤逻辑。
    2.  现在，只要在文章开头的 Frontmatter 区域加上 `published: false`，这篇文章就会被自动隐藏。
*   **用法示例**：
    ```markdown
    ---
    title: "我的秘密日记"
    date: "2025-12-12"
    published: false  <-- 加上这一行，文章就隐藏了
    ---
    ```

### 7. 构建报错修复 (Build Errors)
*   **问题 1：TypeScript 类型错误**
    *   **现象**：运行 `npm run build` 时报错 `Property 'published' does not exist on type...`。
    *   **原因**：在 `src/lib/posts.ts` 中，我们虽然在接口里定义了 `published`，但在读取文件数据时，强制类型转换写得太死 (`as { title: string; date: string }`)，导致 TypeScript 认为读取到的数据里没有 `published` 字段。
    *   **修复**：更新了类型断言，加上了 `published?: boolean`。
*   **问题 2：模块类型警告**
    *   **现象**：构建时提示 `Warning: Module type of file ... is not specified`。
    *   **原因**：Node.js 对 ES Module 和 CommonJS 的混用发出的警告。
    *   **修复**：在 `package.json` 中添加了 `"type": "module"`，明确告诉 Node.js 这是一个 ES Module 项目。
